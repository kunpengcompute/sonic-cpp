...
## 使用示例（使用开源sonic-cpp自带的Benchmark进行性能测试）

sonic-cpp是头文件库，本身无编译产物，测试编译产物在`build-llvm-sve2`文件夹下。

1. 使用开源sonic-cpp自带的Benchmark进行性能测试。
...
sonic-cpp是一个header-only库，使用时只需要包含头文件目录即可。
